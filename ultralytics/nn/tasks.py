...
from .NewModule import PPA,FreqFusion3,MFM
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Detect_ASFF,Detect_AFPN3,Detect_AFPN4)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            c2 = ch[f]
            args = [c2, *args]
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect,Detect_ASFF,Detect_AFPN3,Detect_AFPN4}:
...
def guess_model_scale(model_path):
    ...
    try:
        ...
        return re.search(r"Fide_[v]?\d+([nslmx])", Path(model_path).stem).group(1)  # noqa, returns n, s, m, l, or x
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, v10Detect,Detect_ASFF,Detect_AFPN3,Detect_AFPN4)):
                return "detect"
